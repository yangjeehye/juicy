$(function () {
  ...
  const $sub_on_4 = $('header > .container > nav .lnb > li > .sub_on_4');
    const $container = $('section.special > .visual > .visual-container>li');
    const $pagination = $('section.special > .visual > .visual-pagination > li > a');
    const $prevbtn = $('section.special > .visual-prev');
    const $nextbtn = $('section.special > .visual-next');
  ...
  const fadeFn = function(){
    $container.filter('.on').stop().fadeOut(800).removeClass('on');

    $container.eq(nowIdx).stop().fadeIn(800).addClass('on');
